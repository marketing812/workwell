...
const emotionalChatbotPrompt = ai.definePrompt({
  ...
  // ✅ Fuerza provider+modelo de forma inequívoca (evita defaults raros)
...
const emotionalChatbotFlow = ai.defineFlow(
  ...
  async (rawInput) => {
    // 1) Validación explícita del input
    const input = EmotionalChatbotInputSchema.parse(rawInput);

    // 2) Payload seguro (no pasamos undefined al template)
    ...
    if (!output?.response) {
      throw new Error(
        "Genkit devolvió un 'output' vacío o sin la propiedad 'response'."
      );
    }
