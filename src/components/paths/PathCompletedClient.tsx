...
interface PathCompletedClientProps {
  pathId: string;
}

export function PathCompletedClient({ pathId }: PathCompletedClientProps) {
  const { user } = useUser();
    ...
    <div className="container mx-auto py-8 flex items-center justify-center min-h-[calc(100vh-10rem)]">
      <Card className="w-full max-w-2xl text-center shadow-2xl animate-in fade-in-0 zoom-in-95 duration-500">
        <CardHeader className="pt-10">
          ...
          <CardTitle className="text-4xl font-bold text-primary">¡Ruta completada!</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 px-8 text-lg text-muted-foreground">
          ...
          <p>
            Has completado todos los módulos de la ruta{' '}
            <span className="font-semibold text-accent">"{path?.title || 'seleccionada'}"</span>.
          </p>
          <p className="font-medium text-foreground">¡Sigue así explorando tu bienestar!</p>
        </CardContent>
