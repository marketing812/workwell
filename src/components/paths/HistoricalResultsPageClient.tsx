...
export function HistoricalResultsPageClient({ assessmentId }: HistoricalResultsPageClientProps) {
  ...
  const [rawAnswersWithWeight, setRawAnswersWithWeight] = useState<Record<string, { score: number, weight: number, pesoA?: number, pesoB?: number }> | null>(null);
        ...
        if (record) {
          ...
          if (record.data.respuestas && dimensions.length > 0) {
            const processedAnswers = Object.entries(record.data.respuestas).reduce((acc, [key, value]) => {
              ...
              let pesoA: number | undefined;
              let pesoB: number | undefined;
              ...
              return acc;
            }, {} as Record<string, { score: number, weight: number, pesoA?: number, pesoB?: number }>);
