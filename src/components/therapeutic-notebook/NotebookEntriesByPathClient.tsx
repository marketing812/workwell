...
import { ArrowLeft, Eye, Calendar, BookText, Loader2 } from 'lucide-react';
import { useUser } from '@/contexts/UserContext';
import { pathsData } from '@/data/pathsData';

interface NotebookEntriesByPathClientProps {
  pathId: string;
}

export function NotebookEntriesByPathClient({ pathId }: NotebookEntriesByPathClientProps) {
  ...
  useEffect(() => {
    if (!isNotebookLoading) {
      ...
      const filteredEntries = notebookEntries
        .filter((entry) => entry.pathId === pathId)
    ...
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-primary">Cuaderno terapéutico</h1>
            <p className="text-muted-foreground mt-1">Entradas para la ruta: {pathTitle}</p>
          </div>
        </div>
          ...
          {pathEntries.length > 0 ? (
            pathEntries.map((entry, index) => (
              <Card key={`${entry.id}-${index}`} className="shadow-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <CardContent>
                  <p className="text-sm text-foreground/80 line-clamp-3 whitespace-pre-line break-words">{entry.content}</p>
                </CardContent>
                <CardFooter>
                  <Button asChild variant="outline" className="w-full">
                    <Link href={`/therapeutic-notebook/entry?id=${entry.id}`}>
